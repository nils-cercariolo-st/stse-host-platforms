# STM32H533RE Nucleo host platform

Bare-metal ST1Wire host-platform drivers for the STM32H533RE, targeting the
NUCLEO-H533RE board and its STM32H533RET6 MCU. This branch carries the H523
timer/DMA implementation to the pin-compatible H533 peripheral architecture.
It is an ST1Wire-only platform: I2C support is intentionally excluded. The
timing path is intended for 3-contact (3C) mode; the retained 2C protocol
branches are not a supported or validated target.

## Pin and peripheral assignment

| Function | STM32H533RE resource | Notes |
| --- | --- | --- |
| ST1Wire | PB8, AF2 | CN10 pin 36; open-drain, external pull-up required; PB8 uses VDDIO2 |
...
The branch includes STM32H533 CMSIS device headers and startup files for GCC,
Arm/Keil, and IAR. GCC and IAR linker files target the RE density:
...
`Core/CMSIS/Device/ST/STM32H5xx/Source/gcc/startup_stm32h533xx.s` plus
`Core/CMSIS/Device/ST/STM32H5xx/Source/system_stm32h5xx.c`.

## Hardware validation

This port is compile-checked but has not been exercised on the NUCLEO-H533RE.
Before wiring PB8, confirm the board's VDDIO2 level and select the external
ST1Wire pull-up accordingly. First-board testing should scope PB8 and verify the
SOF, sync/data duty cycles, ACK capture count, and open-drain release level
before running long STSELib transactions.

Primary references:

- [STM32H533RE datasheet](https://www.st.com/resource/en/datasheet/stm32h533re.pdf)
- [NUCLEO-H533RE board](https://www.st.com/en/evaluation-tools/nucleo-h533re.html)
- [STM32H5 Nucleo-64 user manual](https://www.st.com/resource/en/user_manual/dm00941891.pdf)
